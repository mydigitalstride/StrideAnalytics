...
class ECHS_WooCommerce {
	...
	public static function sync_from_woocommerce( int $post_id ): void {
		...
		if ( '' === get_post_meta( $post_id, 'echs_product_price', true ) ) {
			update_post_meta( $post_id, 'echs_product_price', $product->get_price() );
		}

		if ( '' === get_post_meta( $post_id, 'echs_product_currency', true ) ) {
			update_post_meta( $post_id, 'echs_product_currency', get_woocommerce_currency() );
		}

		if ( '' === get_post_meta( $post_id, 'echs_product_availability', true ) ) {
			$avail = $product->is_in_stock() ? 'https://schema.org/InStock' : 'https://schema.org/OutOfStock';
			update_post_meta( $post_id, 'echs_product_availability', $avail );
		}

		if ( '' === get_post_meta( $post_id, 'echs_product_name', true ) ) {
			update_post_meta( $post_id, 'echs_product_name', $product->get_name() );
		}

		if ( '' === get_post_meta( $post_id, 'echs_product_description', true ) ) {
			update_post_meta( $post_id, 'echs_product_description', wp_strip_all_tags( $product->get_description() ) );
		}

		// Ensure Product schema type is enabled.
		$enabled = get_post_meta( $post_id, 'echs_schema_enabled_types', true ) ?: [];
